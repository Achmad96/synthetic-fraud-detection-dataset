...
df_items = pd.DataFrame({
    'ItemID': item_ids,
    'ItemName': [f'Product {i}' for i in item_ids],
    'Price': item_prices
})
...
true_amounts = df_purchased_items.groupby('TransactionID')['LineTotal'].sum().reindex(transaction_ids, fill_value=0).values.astype(float)
